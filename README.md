...
1. Load image → convert to grayscale → handle transparency → apply autocontrast → crop to bounding box → fit to NxN canvas (aspect-ratio preserved, centered on white background)
2. Binarize pixels (threshold at 128) → create a flat binary vector
3. Compare against stored character templates using pixel difference scoring
4. Lowest difference score wins → that's your prediction, with a confidence percentage
...
```python
# tweak the grid size
analyze(path, width=16, height=16)
```
